#!/usr/bin/env python3
"""Pull the Revising/Editing items out of ../shsat-practice/bank.js, tag each with a
grammar concept, and write items.js (ITEMS + PASSAGES) for the grammar app."""
import json, re, collections, sys, os
src = sys.argv[1] if len(sys.argv) > 1 else os.path.join(os.path.dirname(__file__), '..', '..', 'shsat-practice', 'bank.js')
